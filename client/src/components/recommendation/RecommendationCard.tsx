  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardTitle className="pt-3">{item.university.name}</CardTitle>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between text-sm">
          <span>ML recommendation score</span>
          <Badge variant="outline">{item.ml_score.toFixed(2)}</Badge>
        </div>
        <MatchScore
          score={item.match.match_score}
          coverage={item.match.coverage}
          available={item.match.available_criteria}
          total={item.match.total_criteria}
          previous={previousScore}
        />
        <MatchBreakdown match={item.match} />
        <CostBreakdown cost={item.cost} />
